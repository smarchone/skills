...
**No implementations**. Update the single shared `docs/domain/mermaid.md` with all domains' ports.
...
- **Invariants:** Must live in the aggregate root. Every invariant names the error it returns — that error is how the rule is found from the code.
...
8. Write the sketch to `docs/domain/{{domain}}.md`.
9. Update `docs/domain/mermaid.md` (add or refresh subgraphs for all domains). Refer to `mermaid-diagram.md` in this skill for the diagram format and conventions.

## Output Template (`docs/domain/{{domain}}.md`)
...
| Invariant | Error |
|---|---|
| <invariant in canonical terms> | `Err<Condition>` |
...
## Hand-off
Write the file, then update `docs/domain/mermaid.md` following the formatting conventions in `mermaid-diagram.md`. Do not implement the code unless requested.
