---
name: feature
description: Capture a new feature for a hexagonal-architecture project as docs/feature/<name>.md — scenarios, rules, edge cases, and any new business terms it introduces. Use whenever the user wants to add, scope, or clarify a feature or business requirement, or says things like "add a feature", "we also need to support X", "here's what the product should do", or just describes new business behaviour — even if they never mention docs or features. This skill produces one feature document and nothing else; /model turns it into the product, language, and domain snapshots afterwards.
---

# feature

A feature is the unit of change and the only place business intent is written down first-hand. Everything else in `docs/` is a snapshot modelled from features by `/model`.
...
1. **Read for vocabulary.** `docs/language.md` and `docs/product.md` if they exist, plus recent files in `docs/feature/`. You need to know which words are already spoken for. If none of this exists, this is the first feature — proceed, and `/model` will establish those files later.
...
Once the user confirms the feature, `/model` updates `product.md`, `language.md`, and the domain specs from it.
